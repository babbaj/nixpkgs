...
rec {
  ...
  */
  ...
  # High priority functions get applied first (mkBefore), low priority functions get applied last (mkAfter) and return the final result.
  applyAll = functions: x:
    if functions != [] then
      applyAll (tail functions) ((head functions) x)
    else
      x;
